...
public class NewShieldVariantsNeoForge {
  ...
  public NewShieldVariantsNeoForge(final IEventBus modEventBus) {
    ...
    EVENT_BUS.addListener((Consumer<FMLCommonSetupEvent>) event -> NewShieldVariants.init());

    NeoForge.EVENT_BUS.addListener((Consumer<AddServerReloadListenersEvent>) event -> {
      event.addListener(NewShieldVariants.identifier(Constants.MOD_ID), new ResourceReloadListener());
    });

    if (FMLLoader.getCurrent().getDist() == Dist.CLIENT) {
      new NewShieldVariantsClientNeoForge(EVENT_BUS);
    }
  }
